...
function handleAddToCart(p: any) {
  ...
  const ProductCard = ({ p }: { p: any }) => (
    <div className="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] transition-all duration-300 transform hover:-translate-y-1 flex flex-col h-full shrink-0 w-[240px] sm:w-[280px]">
      ...
      <div className="p-5 flex flex-col flex-1 justify-between bg-gradient-to-b from-transparent to-gray-50/30">
        <div>
        ...
        <div className="flex items-center gap-2 mt-4">
          ...
          {p.compare_at_price && <span className="text-sm font-bold text-gray-300 line-through decoration-rose-300">{formatPrice(p.compare_at_price)}</span>}
        </div>
      </div>
    </div>
  );
    ...
    switch (blockId) {
      case 'hero':
        return (
          <>
            {/* ═══ HERO SLIDER (PREMIUM) ═══ */}
            <section className="relative h-[85vh] min-h-[600px] max-h-[900px] overflow-hidden bg-black flex group/hero">
              {bannersLoading ? (
                <BannerSkeleton />
              ) : (
                <>
                  {banners.map((b, i) => (
                    <div key={b.id} className={`absolute inset-0 transition-all duration-1000 ease-out origin-center ${i === heroIdx ? 'opacity-100 scale-100 z-10' : 'opacity-0 scale-105 z-0'}`}>
                      ...
                      <img src={b.image_url} alt={b.title} className="absolute inset-0 w-full h-full object-cover opacity-90 object-center" />
                      ...
                      <div className="relative z-20 max-w-7xl mx-auto px-6 h-full flex flex-col justify-center items-start">
                        <div className="max-w-2xl transform transition-all duration-1000 delay-300 translate-y-0 opacity-100">
                          <span className="inline-block px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-md text-xs font-black uppercase tracking-widest text-white/90 mb-6 border border-white/20">
                            <Sparkles className="w-3.5 h-3.5 inline mr-2 text-primary-400" />
                          ...
                          <p className="text-lg md:text-xl text-gray-300 font-medium max-w-lg leading-relaxed mb-8 drop-shadow-md">
                            {b.subtitle}
                          </p>
                          <Link to={b.link_url || '/shop'} className="group/btn relative inline-flex items-center justify-center overflow-hidden rounded-xl bg-primary-600 px-8 py-4 font-bold text-white shadow-xl transition-all hover:scale-105 hover:shadow-primary-500/30">
                  ...
                  {banners.length > 1 && (
                    <>
                      <button onClick={() => setHeroIdx(i => (i - 1 + banners.length) % banners.length)} className="absolute left-6 top-1/2 -translate-y-1/2 z-30 w-12 h-12 bg-black/20 hover:bg-black/40 border border-white/10 rounded-full flex items-center justify-center backdrop-blur-md transition-all opacity-0 group-hover/hero:opacity-100 -translate-x-4 group-hover/hero:translate-x-0"><ChevronLeft className="w-6 h-6 text-white" /></button>
                      <button onClick={() => setHeroIdx(i => (i + 1) % banners.length)} className="absolute right-6 top-1/2 -translate-y-1/2 z-30 w-12 h-12 bg-black/20 hover:bg-black/40 border border-white/10 rounded-full flex items-center justify-center backdrop-blur-md transition-all opacity-0 group-hover/hero:opacity-100 translate-x-4 group-hover/hero:translate-x-0"><ChevronRight className="w-6 h-6 text-white" /></button>
                      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-30 flex gap-3">
                        {banners.map((_, i) => (<button key={i} onClick={() => setHeroIdx(i)} className={`relative flex h-2 transition-all duration-500 rounded-full overflow-hidden ${i === heroIdx ? 'w-12 bg-white/20' : 'w-2 bg-white/40 hover:bg-white/60'}`}>
                          {i === heroIdx && <div className="absolute inset-y-0 left-0 bg-white w-full animate-[progress_6s_linear]" />}
                        </button>))}
                      </div>
                    </>
                  )}
            ...
            <section className="relative z-20 -mt-10 max-w-7xl mx-auto px-4 sm:px-6">
              <div className="bg-white/80 backdrop-blur-xl border border-white shadow-xl rounded-2xl grid grid-cols-2 md:grid-cols-4 gap-4 p-6 overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-r from-primary-50/50 via-transparent to-primary-50/50 opacity-50 pointer-events-none" />
                {[
                  ...
                  <div key={i} className="flex flex-col items-center justify-center text-center p-2 group">
                    <div className="w-12 h-12 bg-primary-50 text-primary-600 rounded-full flex items-center justify-center mb-3 group-hover:scale-110 group-hover:bg-primary-600 group-hover:text-white transition-all duration-300 shadow-sm">
                      <t.icon className="w-5 h-5" />
                    </div>
                    <span className="text-sm font-black text-gray-900 uppercase tracking-wide">{t.text}</span>
          ...
          <section className="max-w-7xl mx-auto px-6 py-24">
            ...
            {catsLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-[600px]">
                 {categories[0] && (
                   <Link to={`/shop?category=${categories[0].slug}`} className="group relative rounded-3xl overflow-hidden bg-gray-900 md:col-span-2 h-[300px] md:h-full">
                     {categories[0].image_url && <img src={categories[0].image_url} alt={categories[0].name} className="absolute inset-0 w-full h-full object-cover opacity-70 group-hover:opacity-100 group-hover:scale-105 transition-all duration-700" />}
                     <div className="absolute inset-0 bg-gradient-to-t from-gray-900 via-gray-900/40 to-transparent" />
                     <div className="absolute bottom-0 left-0 p-8">
                       <span className="px-3 py-1 bg-primary-600 text-white text-[10px] font-black uppercase tracking-widest rounded-full mb-3 inline-block shadow-lg">{t('home.mostPopular') || 'Más Popular'}</span>
                 ...
                 <div className="flex flex-col gap-6 h-[300px] md:h-full">
                   {categories.slice(1, 3).map(c => (
                     <Link key={c.id} to={`/shop?category=${c.slug}`} className="group relative rounded-3xl overflow-hidden bg-gray-900 flex-1">
                       {c.image_url && <img src={c.image_url} alt={c.name} className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:opacity-80 group-hover:scale-105 transition-all duration-700" />}
        ...
        return (
          <>
            {groups.map((g, i) => g.product_group_items && g.product_group_items.length > 0 && (
              <section key={g.id} className={`py-20 ${i % 2 === 0 ? 'bg-white border-y border-gray-100' : ''}`}>
                <div className="max-w-7xl mx-auto px-6">
                  <div className="flex items-center justify-between mb-10">
                    <div>
                    ...
                    <Link to={`/shop?group=${g.slug}`} className="hidden md:flex items-center gap-2 text-sm font-bold text-gray-900 hover:text-primary-600 px-5 py-2.5 rounded-full border-2 border-gray-900 hover:border-primary-600 transition-colors">
      ...
      case 'trending':
        return (
          <section className="bg-white py-20 border-y border-gray-100">
             <div className="max-w-7xl mx-auto px-6">
                <div className="flex items-center justify-between mb-10">
                  <h2 className="text-3xl font-black text-gray-900 tracking-tight">{t('home.featured')}</h2>
                  <Link to="/shop" className="hidden md:flex items-center gap-2 text-sm font-bold text-gray-900 hover:text-primary-600 px-5 py-2.5 rounded-full border-2 border-gray-900 hover:border-primary-600 transition-colors">
                    {t('home.viewAll')} <ArrowRight className="w-4 h-4" />
                  </Link>
                </div>
                <div className="flex overflow-x-auto gap-6 pb-8 snap-x snap-mandatory scrollbar-hide -mx-6 px-6">
            ...
            {brandsLoading ? (
              ...
              <section className="py-24 overflow-hidden bg-gray-50 relative">
                ...
                <div className="max-w-7xl mx-auto px-6 mb-12 text-center relative z-10">
                  <h2 className="text-sm font-black text-primary-600 mb-2 uppercase tracking-[0.2em]">Sponsors & Partners</h2>
                  <h3 className="text-3xl md:text-4xl font-black text-gray-900 tracking-tight">
                    {t('home.brands') || 'Tus marcas de siempre'}
                  </h3>
                </div>
                <div className="relative w-full overflow-hidden z-10 flex border-y border-gray-200 bg-white py-8 shadow-sm">
                   ...
                   <div className="flex animate-[slide_30s_linear_infinite] whitespace-nowrap min-w-max hover:[animation-play-state:paused] items-center">
                      {[...brands, ...brands, ...brands].map((b, i) => (
                         <Link key={`${b.id}-${i}`} to={`/shop?brand=${b.slug}`} className="mx-12 shrink-0 group transition-transform hover:scale-110">
                            {b.logo_url ? (
                               <img src={b.logo_url} alt={b.name} className="h-10 md:h-12 w-auto object-contain filter grayscale opacity-40 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300" />
                            ) : (
                               <span className="text-xl md:text-2xl font-black text-gray-300 group-hover:text-primary-500 uppercase tracking-widest">{b.name}</span>
                            )}
                         </Link>
                      ))}
                   </div>
  ...
  return (
    <div className="bg-gray-50/50">
      ...
      {/* ═══ VIP NEWSLETTER BANNER (Static Bottom) ═══ */}
      <section className="bg-dark-900 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 py-24 relative z-10 flex flex-col md:flex-row items-center justify-between gap-12 text-center md:text-left">
          <div className="md:w-1/2">
            <h2 className="text-4xl md:text-5xl font-black text-white mb-4 tracking-tight -rotate-2 transform">Join the Elite Collector's Club</h2>
            <p className="text-lg text-gray-400 font-medium">Suscríbete para acceso anticipado a nuevas oleadas, drops limitados, y un 10% OFF en tu primera compra premium.</p>
          </div>
          <div className="md:w-1/2 w-full max-w-md bg-white/5 backdrop-blur-md p-8 rounded-3xl border border-white/10 shadow-2xl">
            <form className="flex flex-col gap-4" onSubmit={e => { e.preventDefault(); alert('Subscribed!'); }}>
              <input type="email" placeholder="Ingresa tu mejor email..." className="px-6 py-4 rounded-xl bg-black/40 text-white border border-white/20 placeholder:text-gray-500 focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none text-center font-medium transition-colors" required />
              <button type="submit" className="px-6 py-4 rounded-xl bg-white text-dark-900 font-black tracking-widest uppercase hover:bg-primary-500 hover:text-white transition-all transform hover:-translate-y-1 shadow-[0_0_20px_rgba(255,255,255,0.2)] hover:shadow-primary-500/40">Desbloquear Beneficios</button>
            </form>
            <p className="text-xs text-gray-500 mt-4 text-center">Unsubscribe at any time. We respect your privacy.</p>
          </div>
        </div>
      </section>

      {/* Put custom CSS animations in style tag for the sliders */}
