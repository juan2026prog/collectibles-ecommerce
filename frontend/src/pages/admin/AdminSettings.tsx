...
import { Save, ToggleLeft, ToggleRight, Settings, Store, Truck, Palette, LayoutTemplate, Plus, Trash2, ChevronUp, ChevronDown, GripVertical, FileText, Share2, Link as LinkIcon } from 'lucide-react';
...
export default function AdminSettings() {
  const [tab, setTab] = useState<'general' | 'modules' | 'shipping' | 'appearance' | 'social'>('general');
    ...
    <div>
      ...
      <div className="flex gap-1 mb-6 bg-gray-100 rounded-lg p-1 w-fit overflow-x-auto">
        {[
          ...
          { key: 'modules', label: 'Módulos Activos', icon: Settings },
      ...
      {tab === 'general' && (
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-6 max-w-2xl shadow-sm">
          ...
          {[
            ...
            { key: 'currency', label: 'Moneda (Código)' },
            { key: 'currency_symbol', label: 'Símbolo Moneda' },
            { key: 'free_shipping_threshold', label: 'Monto Envío Gratis ($)' },
            { key: 'default_shipping_rate', label: 'Tarifa Envío Base ($)' },
          ].map(field => (
