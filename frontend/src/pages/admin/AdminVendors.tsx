      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="overflow-x-auto">
          <ResponsiveDataList
            ...
            keyExtractor={(v) => v.id}
            loading={loading}
            emptyTitle="No se encontraron vendors"
            ...
            renderCard={(v) => {
              ...
              return (
                <div key={v.id} className="p-4 bg-white border border-gray-200 rounded-xl space-y-3 shadow-xs">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-12 h-12 rounded-lg border border-gray-200 overflow-hidden bg-white flex items-center justify-center shrink-0">
                        {v.logo_url ? <img src={v.logo_url} alt="" className="w-full h-full object-cover" /> : <Store className="w-6 h-6 text-gray-300" />}
                      </div>
                      <div className="min-w-0">
                        <h4 className="font-bold text-gray-900 text-sm truncate">{v.store_name}</h4>
                        <p className="text-xs text-gray-500 truncate">{v.company_name || 'Razón social no especificada'}</p>
                      </div>
                    </div>

                    <span className={`px-2.5 py-1 rounded-full text-xs font-bold shrink-0 ${
                      v.status === 'active' ? 'bg-teal-50 text-teal-800 border border-teal-200' :
                      v.status === 'suspended' ? 'bg-amber-50 text-amber-800 border border-amber-200' :
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs border-t border-b border-gray-100 py-2">
                    <div>
                      ...
                      <span className="font-medium text-gray-800 truncate block">{v.profiles?.email || '—'}</span>
                    </div>
                    <div>
                      <span className="text-gray-400 block text-[10px] uppercase font-semibold">Teléfono</span>
                      <span className="font-medium text-gray-800 truncate block">{v.contact_phone || v.profiles?.phone || '—'}</span>
                    </div>
                    <div>
                      <span className="text-gray-400 block text-[10px] uppercase font-semibold">GMV Confirmado</span>
                      <span className="font-bold text-emerald-600 text-sm">${Number(v.metrics?.confirmed_gmv || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                    <div>
                      <span className="text-gray-400 block text-[10px] uppercase font-semibold">Productos</span>
                      <span className="font-bold text-gray-900">{prodCount} u.</span>
                    </div>
                  </div>

                  <div className="flex items-center justify-between pt-1">
                    <span className="text-[10px] text-gray-400 font-medium">
                      Alta: {v.created_at ? new Date(v.created_at).toLocaleDateString('es') : '—'}
                    </span>

                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => setSelectedVendor(v)}
                        className="px-3 py-1.5 bg-teal-50 hover:bg-teal-100 text-teal-800 font-bold rounded-lg text-xs transition-colors min-h-[36px]"
                      >
                        Detalles
                      </button>
                      <button
            ...
            renderTableRow={(v) => {
              ...
              return (
                <tr key={v.id} className="hover:bg-gray-50 transition-colors">
                  <td className="p-4 font-bold text-gray-900">{v.store_name}</td>
                  <td className="p-4 text-xs font-semibold text-gray-500">{v.profiles?.email || '—'}</td>
                  <td className="p-4 text-xs font-semibold text-gray-500">{v.contact_phone || v.profiles?.phone || '—'}</td>
                  <td className="p-4">
                    <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-bold ${
                      v.status === 'active' ? 'bg-teal-50 text-teal-800 border border-teal-200' : 'bg-slate-100 text-slate-700'
                    }`}>
                      {v.status || 'active'}
                    </span>
                  </td>
                  <td className="p-4 text-xs font-bold">{v.kyc_status || 'Pendiente'}</td>
                  <td className="p-4 text-xs font-semibold">{v.vendor_stores?.length || 0} tiendas</td>
                  <td className="p-4 font-bold text-emerald-600">${Number(v.metrics?.confirmed_gmv || 0).toLocaleString()}</td>
                  <td className="p-4 text-center font-bold">{prodCount}</td>
                  <td className="p-4 text-center text-xs">{mlConnected ? 'Sí' : 'No'}</td>
                  <td className="p-4 text-center text-xs">{v.ships_to_argentina ? 'Sí' : 'No'}</td>
                  <td className="p-4 text-xs text-gray-500">{v.created_at ? new Date(v.created_at).toLocaleDateString('es') : '—'}</td>
                  <td className="p-4 text-right">
                    <button onClick={() => setSelectedVendor(v)} className="px-3 py-1.5 text-xs font-bold text-teal-700 bg-teal-50 rounded-lg">Detalles</button>
                  </td>
                </tr>
              );
            }}
