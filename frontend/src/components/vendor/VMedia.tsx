...
export default function VMedia() {
  ...
  const VENDOR_ROOT = user ? `vendors/${user.id}/` : '';
  ...
  async function fetchMedia(path: string) {
    if (!VENDOR_ROOT) return;
    setLoading(true);
    setError(null);
    setFiles([]); 
    try {
      const { data, error } = await supabase.storage.from(BUCKET_NAME).list(VENDOR_ROOT + path, {
        limit: 500,
        offset: 0,
  ...
  async function handleCreateFolder() {
    ...
    const placeholderPath = `${VENDOR_ROOT}${folderPath}/.emptyFolderPlaceholder`;
  ...
  async function uploadFiles(filesArray: File[], destPath?: string) {
    ...
    for (const file of filesArray) {
      try {
        ...
        const fullPath = targetPath ? `${VENDOR_ROOT}${targetPath}${fileName}` : `${VENDOR_ROOT}${fileName}`;
  ...
  async function handleFolderDrop(e: React.DragEvent, targetFolderName: string) {
    ...
    try {
      if (isFolder) {
        await moveFolderRecursive(VENDOR_ROOT + fromPath + '/', VENDOR_ROOT + toPath + '/');
      } else {
        const { error } = await supabase.storage.from(BUCKET_NAME).move(VENDOR_ROOT + fromPath, VENDOR_ROOT + toPath);
        if (error) throw error;
      }
  ...
  async function handleDelete(fileName: string, isFolder: boolean) {
    ...
    try {
      if (isFolder) {
        await deleteFolderRecursive(VENDOR_ROOT + targetPath + '/');
      } else {
        const { error } = await supabase.storage.from(BUCKET_NAME).remove([VENDOR_ROOT + targetPath]);
        if (error) throw error;
      }
      fetchMedia(currentPath);
  ...
  async function handleMove(fileName: string, isFolder: boolean) {
    const targetFolder = await prompt(
      `Mover ${isFolder ? 'la carpeta' : 'el archivo'} a la carpeta de destino (dentro de tu espacio):`,
      currentPath
    );
  ...
  function getFileUrl(fileName: string) {
    const fullPath = currentPath ? `${VENDOR_ROOT}${currentPath}${fileName}` : `${VENDOR_ROOT}${fileName}`;
    ...
    <div className="space-y-6 text-gray-900 animation-fade-in">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-gray-100 pb-4">
        <div>
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <ImageIcon className="w-6 h-6 text-blue-600" /> Mi Biblioteca Multimedia
          </h2>
          {/* Breadcrumbs */}
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => fetchMedia(currentPath)} className="p-2.5 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors shadow-sm" title="Recargar">
            <RefreshCw className={`w-4 h-4 text-gray-600 ${loading ? 'animate-spin' : ''}`} />
          </button>
          
          <button onClick={handleCreateFolder} className="px-4 py-2 bg-white border border-gray-200 rounded-lg font-bold flex items-center gap-2 hover:bg-gray-50 transition-colors shadow-sm">
            <FolderPlus className="w-4 h-4 text-gray-600" /> Nueva Carpeta
          </button>
          ...
          <button 
            ...
            disabled={uploading}
            className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2 transition-colors shadow-md disabled:opacity-50"
          >
      ...
      <div className="bg-white border border-gray-200 p-4 rounded-xl shadow-sm">
        <div className="relative max-w-md">
          <input
            ...
            className="w-full pl-10 pr-10 py-2 border border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none"
          />
      ...
      <div 
        ...
        className={`border-2 border-dashed p-6 text-center transition-all rounded-xl shadow-sm ${
          isDragging ? 'border-blue-500 bg-blue-50/50 scale-[1.01]' : 'border-gray-200 bg-white hover:border-blue-400'
        }`}
      >
        {uploading ? (
          ...
          <p className="text-sm text-gray-500">
            <span className="font-bold text-blue-600">Arrastra y suelta</span> archivos aquí, o 
            <button onClick={() => fileInputRef.current?.click()} className="text-blue-600 hover:underline ml-1 font-bold">selecciona desde tu equipo</button>
          </p>
        )}
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm min-h-[400px] p-4">
        {loading && files.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64 text-gray-400">
            <RefreshCw className="w-8 h-8 animate-spin mb-4 text-blue-300" />
            <p className="font-medium">Cargando biblioteca...</p>
          </div>
        ) : files.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64 text-center p-6">
            ...
            <p className="text-gray-500 mt-1 max-w-sm text-sm">No hay archivos en esta ubicación. Sube uno o crea una carpeta nueva para organizarte.</p>
          </div>
        ) : (
              ...
              return filtered.map(file => {
                   ...
                   return (
                      ...
                      >
                         <button 
                           ...
                           className="w-full h-full flex flex-col items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed outline-none"
                         >
                         ...
                         <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                           <button onClick={(e) => { e.stopPropagation(); handleRename(file.name, true); }} className="p-1 text-gray-500 hover:text-blue-600 hover:bg-blue-50 bg-white shadow-sm border border-gray-200 rounded-md" title="Renombrar">
                             <Edit className="w-3.5 h-3.5" />
                           </button>
                           <button onClick={(e) => { e.stopPropagation(); handleMove(file.name, true); }} className="p-1 text-gray-500 hover:text-green-600 hover:bg-green-50 bg-white shadow-sm border border-gray-200 rounded-md" title="Mover">
                             <Move className="w-3.5 h-3.5" />
                           </button>
                           <button onClick={(e) => { e.stopPropagation(); handleDelete(file.name, true); }} className="p-1 text-gray-500 hover:text-red-500 hover:bg-red-50 bg-white shadow-sm border border-gray-200 rounded-md" title="Eliminar">
                ...
                return (
                  <div 
                    ...
                    className="group relative border border-gray-200 rounded-xl flex flex-col items-center justify-center p-2 hover:border-blue-400 hover:shadow-md transition-all bg-white aspect-square overflow-hidden cursor-grab active:cursor-grabbing"
                  >
