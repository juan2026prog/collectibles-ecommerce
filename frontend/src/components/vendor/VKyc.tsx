...
export default function VKyc() {
  ...
  if (loading) return <div className="text-white">Cargando datos KYC...</div>;

  return (
    <div className="space-y-8 animation-fade-in max-w-4xl">
      <div>
         <div className="text-[11px] text-[#f00856] font-black uppercase tracking-[0.4em] mb-3">Identity Verification</div>
         ...
         <p className="text-sm text-slate-500 font-bold mt-3 uppercase tracking-[0.2em]">Verificación requerida para operar el marketplace</p>
      </div>
      ...
      <div className="glass rounded-[2rem] border border-white/10 p-8 shadow-2xl relative overflow-hidden">
        ...
        <form onSubmit={handleSave} className="space-y-6 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Razón Social / Nombre Comercial</label>
              <input
                type="text"
                value={companyName}
                onChange={e => setCompanyName(e.target.value)}
                disabled={vendor?.kyc_status === 'approved'}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#f00856] transition-colors disabled:opacity-50"
                placeholder="Ej. Tienda Demo S.R.L."
                required
              />
            </div>
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">RUT / Tax ID</label>
              <input
                type="text"
                value={taxId}
          ...
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Documentos Adjuntos (BETA)</label>
            <div className="border-2 border-dashed border-white/10 rounded-xl p-8 text-center hover:border-white/20 transition-colors">
              ...
              <p className="text-sm font-bold text-slate-400">La subida de archivos directos está deshabilitada en esta demo.</p>
              <p className="text-xs text-slate-500 mt-1">Por favor contacta al administrador para el envío manual de DNI y RUT.</p>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-white/5">
            {vendor?.kyc_status === 'approved' ? (
              ...
              <button
                type="submit"
                disabled={saving}
                className="bg-[#f00856] text-white px-8 py-4 rounded-xl font-black uppercase tracking-[0.2em] text-xs hover:bg-[#d00040] transition-colors disabled:opacity-50"
              >
