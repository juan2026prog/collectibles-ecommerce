        ...
        {mode === 'language' && (
          <label className="flex items-center gap-2">
            ...
            <select
              ...
              className="text-sm border border-white/10  px-2 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-primary-400 cursor-pointer"
            >
              {LANGUAGES.map(l => (
    ...
    <div ref={ref} className={`relative ${className}`}>
      <button
        ...
        className={`flex items-center gap-1.5 ${compact
          ? 'px-2 py-1.5 text-xs font-semibold text-slate-400 hover:text-white hover:bg-white/10  transition-colors'
          : 'px-3 py-2 text-sm font-medium text-gray-700 hover:bg-white/10 border border-white/10  transition-colors'
        }`}
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-72 glass  shadow-xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
          {/* Language section */}
              ...
              {LANGUAGES.map(l => (
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2  text-sm font-semibold transition-all ${
                    language === l.code
                      ? 'bg-primary-500/10 text-primary-700 ring-1 ring-primary-300'
            ...
            <div className="flex flex-col gap-1">
              {CURRENCIES.map(c => (
                <button
                  ...
                  className={`flex items-center gap-3 px-3 py-2  text-sm font-semibold transition-all ${
                    currency === c.code
                      ? 'bg-primary-500/10 text-primary-700 ring-1 ring-primary-300'
                      : 'text-gray-700 hover:bg-white/10'
                  }`}
                >
                  ...
                  <div className="text-left">
                    ...
                    <div className="text-xs text-slate-400 font-normal">{c.label}</div>
                  </div>
