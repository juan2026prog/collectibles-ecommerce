...
export default function S2FProfile({ creatorData, onSave, saving }: Props) {
  ...
  return (
    <div className="max-w-4xl space-y-6 pb-10">
      <div>
        <h2 className="text-3xl font-black text-white tracking-tight mb-1">Mi Perfil & Configuración</h2>
        <p className="text-slate-400 font-medium">Controlá cómo te ven los fans, precios, disponibilidad y reglas de contenido.</p>
      </div>
      ...
      <Section title="Preferencias del Panel (Global)" icon={Settings}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
           <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Idioma del Panel</label>
              <select
                 className="w-full bg-white/5 border border-white/10  px-4 py-3.5 text-sm font-semibold outline-none focus:border-rose-500"
                 value={language}
           ...
           <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Moneda (Visualización)</label>
              <select
                 className="w-full bg-white/5 border border-white/10  px-4 py-3.5 text-sm font-semibold outline-none focus:border-rose-500"
                 value={currency}
      ...
      <Section title="Identidad Pública" icon={User}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="md:col-span-2">
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Biografía Corta</label>
            <textarea value={form.short_bio} onChange={e => update('short_bio', e.target.value)} rows={3}
              className="w-full bg-white/5 border border-white/10  p-3.5 text-white outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-500/10 text-sm resize-none" />
          </div>
          <Field label="País" value={form.country} onChange={v => update('country', v)} placeholder="Argentina" />
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Idiomas</label>
            <input value={(form.languages as string[]).join(', ')} onChange={e => update('languages', e.target.value.split(',').map(s => s.trim()))}
              className="w-full bg-white/5 border border-white/10  p-3.5 font-medium text-white outline-none focus:border-rose-500 text-sm" placeholder="es, en, pt" />
          </div>
        </div>

        <div className="mt-5">
          <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Foto de Perfil & Banner</label>
          <div className="flex gap-4">
            <div className="w-24 h-24 bg-white/10 border-2 border-dashed border-gray-300  flex items-center justify-center cursor-pointer hover:border-rose-400 transition-colors">
              {creatorData?.profile_photo_url
                ? <img src={creatorData.profile_photo_url} className="w-full h-full object-cover " />
                : <Camera className="w-6 h-6 text-slate-500" />}
            </div>
            <div className="flex-1 h-24 bg-white/10 border-2 border-dashed border-gray-300  flex items-center justify-center cursor-pointer hover:border-rose-400 transition-colors">
              {creatorData?.cover_banner_url
                ? <img src={creatorData.cover_banner_url} className="w-full h-full object-cover " />
        ...
        <div className="mt-5">
          <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Redes Sociales (JSON)</label>
          <input value={JSON.stringify(form.social_links)} onChange={e => { try { update('social_links', JSON.parse(e.target.value)); } catch { } }}
            className="w-full bg-white/5 border border-white/10  p-3.5 text-white outline-none focus:border-rose-500 text-xs font-mono" placeholder='{"instagram":"@user","tiktok":"@user"}' />
        </div>
      </Section>
      ...
      <Section title="Precios & Tiempos" icon={DollarSign}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-5">
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Tiempo de Entrega</label>
            <select value={form.estimated_delivery_time} onChange={e => update('estimated_delivery_time', Number(e.target.value))}
              className="w-full bg-white/5 border border-white/10  p-3.5 font-bold text-white outline-none focus:border-rose-500">
      ...
      <Section title="Tipos de Saludo" icon={Tag}>
        <p className="text-sm text-slate-400 mb-4">Seleccioná qué tipo de saludos aceptás.</p>
        <div className="flex flex-wrap gap-2">
          {occasions.map(o => {
            const active = (form.accepted_request_types as string[]).includes(o.id);
            return (
              <button key={o.id} onClick={() => toggleOccasion(o.id)}
                className={`px-4 py-2 rounded-full text-sm font-bold border transition-all ${active ? 'bg-rose-50 border-rose-300 text-rose-700' : 'bg-white/5 border-white/10 text-slate-500 hover:text-slate-400'}`}>
                {active ? '✓ ' : ''}{o.label}
              </button>
            );
          })}
      ...
      <Section title="Disponibilidad" icon={Clock}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Estado Público</label>
          ...
          <div className="flex items-center gap-3 p-3.5 bg-white/5 border border-white/10 ">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Pausar al alcanzar el máximo</p>
            </div>
          </div>
        </div>
      </Section>
      ...
      <Section title="Reglas & Límites de Contenido" icon={Shield}>
        <div className="space-y-5">
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Reglas de Contenido</label>
            <textarea value={form.content_rules} onChange={e => update('content_rules', e.target.value)} rows={3}
              className="w-full bg-white/5 border border-white/10  p-3.5 text-white outline-none focus:border-rose-500 text-sm resize-none"
          ...
          <div>
            ...
            <input value={form.blocked_keywords} onChange={e => update('blocked_keywords', e.target.value)}
              className="w-full bg-white/5 border border-white/10  p-3.5 text-white outline-none focus:border-rose-500 text-sm"
      ...
      <div className="flex justify-end pt-2">
        <button onClick={() => onSave(form)} disabled={saving}
          className="bg-gray-900 text-white font-black px-10 py-4  hover:bg-gray-800 transition-colors shadow-lg disabled:opacity-50 flex items-center gap-2">
  ...
  return (
    <div className="glass shadow-sm overflow-hidden">
      <div className="p-5 lg:p-6">
        <h4 className="flex items-center gap-2 font-black text-white uppercase tracking-widest text-sm mb-5 pb-2 border-b border-white/10">
  ...
  return (
    <div>
      ...
      <input value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
        className="w-full bg-white/5 border border-white/10  p-3.5 font-medium text-white outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-500/10 text-sm" />
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <input type="number" value={value} onChange={e => onChange(Number(e.target.value))}
        className="w-full bg-white/5 border border-white/10  p-3.5 font-black text-white text-lg outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-500/10" />
    </div>
  );
}
