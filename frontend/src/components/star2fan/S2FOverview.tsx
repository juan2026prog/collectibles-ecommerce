...
export default function S2FOverview({ creatorData, requests, earnings, reviews, notifications, onSelectRequest, onChangeTab }: Props) {
  ...
  return (
    <div className="space-y-6">
      {/* Banner */}
      <div className="bg-gradient-to-r from-rose-600 to-pink-700  p-8 lg:p-10 text-white shadow-xl relative overflow-hidden flex flex-col md:flex-row justify-between items-start gap-6">
        ...
        <div className="relative z-10 bg-black/20  p-4 border border-white/10 text-right min-w-[200px]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {kpis.map(k => {
          const Icon = k.icon;
          return (
            <div key={k.label} onClick={() => onChangeTab(k.click)} className="bg-white p-5  border border-white/10 shadow-sm hover:-translate-y-1 hover:shadow-md transition-all cursor-pointer">
              <div className="flex justify-between items-start mb-3">
                <div className={`p-2.5  ${colorMap[k.color]}`}><Icon className="w-5 h-5" /></div>
                <span className="text-2xl font-black text-white">{k.value}</span>
              </div>
              <h3 className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">{k.label}</h3>
            </div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="glass shadow-sm  p-6 lg:col-span-1">
          <h3 className="text-base font-black text-white mb-4 flex items-center gap-2">
            <div className="w-2 h-2 bg-rose-500 rounded-full animate-pulse"></div> Notificaciones
          </h3>
          <div className="space-y-3 max-h-80 overflow-y-auto pr-1">
            {notifications.length === 0 && <p className="text-slate-500 text-sm">Sin notificaciones.</p>}
            {notifications.slice(0, 8).map(n => (
              <div key={n.id} className={`border-l-2 ${n.is_read ? 'border-white/10' : 'border-rose-500'} pl-3 py-1`}>
        ...
        <div className="glass shadow-sm  overflow-hidden lg:col-span-2">
          <div className="p-5 border-b border-white/10 bg-white/5 flex justify-between items-center">
            <h3 className="text-base font-black text-white">Atención Requerida</h3>
          ...
          <div className="divide-y divide-gray-50">
            {active.slice(0, 5).map(req => (
              <div key={req.id} onClick={() => { onSelectRequest(req); onChangeTab('requests'); }}
                className="p-4 hover:bg-white/5 transition-colors cursor-pointer flex justify-between items-center gap-4">
                <div className="flex items-center gap-3">
                  <div className={`p-2  ${req.priority === 'urgent' ? 'bg-red-50 text-red-600' : 'bg-rose-50 text-rose-600'}`}>
                    {req.priority === 'urgent' ? <AlertCircle className="w-4 h-4" /> : <Video className="w-4 h-4" />}
                  </div>
                  <div>
                    <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">{req.occasion}</p>
                    <p className="text-sm font-black text-white">De {req.fan_buyer_name} → {req.recipient_name}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 text-right">
                  <div>
                    <p className="text-lg font-black text-white">${Number(req.price).toFixed(0)}</p>
                    <p className={`text-[10px] font-bold uppercase ${new Date(req.delivery_deadline) < new Date() ? 'text-red-500' : 'text-slate-500'}`}>
            ...
            {active.length === 0 && <div className="p-8 text-center text-slate-500 text-sm">No hay solicitudes pendientes 🎉</div>}
          </div>
        </div>
      </div>
    </div>
