...
export default function S2FSupport() {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        <h2 className="text-3xl font-black text-white tracking-tight mb-1">Centro de Ayuda</h2>
        <p className="text-slate-400 font-medium">Todo lo que necesitás saber para ser un creador exitoso en Star2Fan.</p>
      </div>

      {/* Tips Grid */}
      <div className="bg-gradient-to-r from-rose-50 to-pink-50 border border-rose-200  p-6">
        <h3 className="text-base font-black text-white mb-4 flex items-center gap-2"><Video className="w-5 h-5 text-rose-500" /> Tips para Grabar Mejores Videos</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {tips.map(t => (
            <div key={t.title} className="bg-white/80 backdrop-blur-sm  p-4 border border-rose-100">
      ...
      <div className="glass  shadow-sm overflow-hidden">
        <div className="p-5 border-b border-white/10 bg-white/5 flex items-center gap-2">
          ...
          <h3 className="text-base font-black text-white">Preguntas Frecuentes</h3>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {guidelines.map(g => (
          <div key={g.title} className="glass  p-5 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass  p-5 shadow-sm">
          ...
          <p className="text-sm text-slate-400 mb-4">¿Tenés un problema que no se resuelve con las FAQs?</p>
          <button className="bg-rose-600 text-white font-black py-3 px-6  hover:bg-rose-500 transition-colors w-full shadow-md">
            Enviar Ticket de Soporte
          </button>
          <p className="text-xs text-slate-500 text-center mt-2">soporte@star2fan.app</p>
        </div>
      </div>
    </div>
  );
...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="px-5 py-4 cursor-pointer hover:bg-white/5 transition-colors" onClick={() => setOpen(!open)}>
