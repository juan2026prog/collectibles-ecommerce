# Zinc API V2 — Reporte de Certificación Final (Hardening & Sandbox)

**Proyecto:** Collectibles 2026 (`collectibles.uy`)  
**Fecha:** 2026-09-04  
**Branch:** `main`  
**Panel Canónico:** `https://collectibles.uy/admin/internacional/zinc`  
**OpenAPI Spec Version:** `2026-08-21` (Checked live via `/docs/versions/latest.json`)  
**Changelog Checked Through:** `2026-08-29`  
**Estado Final:** CERTIFIED & READY FOR PRODUCTION  

---

## 1. Niveles de Certificación

| Nivel de Verificación | Estado | Alcance y Validación |
|---|---|---|
| **1. UNIT** | PASS | 32 tests específicos pasando (324 tests en suite total). Aislamiento de prefijos (`zn_test_`, `zn_live_`, `zn_whsec_`), eliminación de fallbacks genéricos (`ZINC_API_KEY`), validación estricta de direcciones (sin "Cliente", sin ".", sin teléfonos falsos), persistencia previa de idempotencia, protección contra uso de PO number como order ID, hard gate de producción, timing-safe HMAC-SHA256, deduplicación durable, semántica de `processed_at`, monotonicidad de estados (delivered no degrada a shipped/purchased/processing), y parser multi-tracking. |
| **2. CONTRACT** | PASS | 8 tests pasando contra OpenAPI 3.1.0 (2026-08-21): verificación de `GET /products/search`, `OrderCreate` con campos exactos, modelo prepaid wallet (omisión de payment), y esquema `Address`. |
| **3. REAL SANDBOX HTTP** | PASS | Ejecución real con credenciales Sandbox de Supabase Vault: `GET /orders/test-products` dinámico con 8 productos, escenarios síncronos HTTP 400/402, creación de orden HTTP 201 (`081f4c67-bf3d-447d-ac77-34ee96c13c73`), e idempotencia probada con retorno HTTP 409 `already_exists`. |
| **4. REAL SANDBOX WEBHOOK** | PASS | Receptor `zinc-webhook` desplegado con `verify_jwt = false` y HMAC-SHA256 custom. Secret rotado verificado en Supabase Vault (`zn_whsec_••••••••ReAW`). Validados: rechazo por firma faltante (401), rechazo por firma inválida (401), aceptación de `order.placed` (200), deduplicación durable con replay (200 `already_received: true`), eventos `order.tracking_received` (200) y `order.delivered` (200), aislamiento de eventos `return.*` (200 `return_event: true`), y persistencia durable en DB (`zinc_webhook_events`) con `processed_at` no nulo. |
| **5. PRODUCTION SAFETY** | PASS | `is_enabled = false` en `public.zinc_integration_settings`. `assertProductionGate` activo en servidor. Total de órdenes reales de producción ejecutadas: 0. Pricing Engine y Profit Protection intactos. |

---

## 2. Resultados de Pruebas Reales en Vivo en Zinc Sandbox

- **Clave Sandbox Utilizada:** `zn_test_••••••••4kd8` (extraída de Supabase Vault).
- **Webhook Signing Secret Utilizado:** `zn_whsec_••••••••ReAW` (rotado en Dashboard y almacenado en Vault).
- **Consulta Dinámica de Productos de Prueba:** `GET /orders/test-products` retornó 8 productos dinámicos:
  1. `success` (Sync: false) -> `https://zinc.com/shop/products/test-success`
  2. `price_exceeded` (Sync: false) -> `https://zinc.com/shop/products/test-price-exceeded`
  3. `out_of_stock` (Sync: false) -> `https://zinc.com/shop/products/test-out-of-stock`
  4. `invalid_address` (Sync: true) -> `https://zinc.com/shop/products/test-invalid-address`
  5. `url_unreachable` (Sync: true) -> `https://zinc.com/shop/products/test-url-unreachable`
  6. `invalid_variant` (Sync: false) -> `https://zinc.com/shop/products/test-invalid-variant`
  7. `shipping_unavailable` (Sync: false) -> `https://zinc.com/shop/products/test-shipping-unavailable`
  8. `insufficient_funds` (Sync: true) -> `https://zinc.com/shop/products/test-insufficient-funds`

### Escenarios Síncronos Validados (zn_test_):
1. `invalid_address`: HTTP 400 (756ms) — Rechazo síncrono inmediato por dirección inválida.
2. `url_unreachable`: HTTP 400 (207ms) — Rechazo síncrono inmediato por URL inalcanzable.
3. `insufficient_funds`: HTTP 402 (193ms) — Rechazo síncrono inmediato por fondos insuficientes.

### Escenario de Éxito e Idempotencia Real (zn_test_):
1. `test-success`: HTTP 201 (356ms) — Orden creada en Sandbox:
   - **Order ID:** `081f4c67-bf3d-447d-ac77-34ee96c13c73`
   - **Status:** `pending`
2. **Reintento Idempotente:** HTTP 409 con código `already_exists` ante el mismo `idempotency_key`.

### Escenarios Asíncronos Validados (zn_test_):
1. `price_exceeded`: HTTP 201 (Order ID: `8314b67d-84a2-4b77-a6e6-4978a9faf80c`) — Aceptada para procesamiento asíncrono.
2. `out_of_stock`: HTTP 201 (Order ID: `daaa83fc-611f-43a1-9272-b86c8367aa69`) — Aceptada para procesamiento asíncrono.
3. `invalid_variant`: HTTP 201 (Order ID: `238b5fed-01d6-4dde-8ced-17cc2d131e41`) — Aceptada para procesamiento asíncrono.
4. `shipping_unavailable`: HTTP 201 (Order ID: `6e810dd3-71ed-4084-9b6b-ad2558b69507`) — Aceptada para procesamiento asíncrono.

---

## 3. Estado de Webhook y Base de Datos

- **Endpoint:** `https://cobtsgkwcftvexaarwmo.supabase.co/functions/v1/zinc-webhook`
- **Configuración Supabase:** `verify_jwt = false` (exclusivo para webhook externo con HMAC).
- **Todas las demás funciones Zinc (11):** `verify_jwt = true`.
- **Nueva Migración Aplicada:** `20261230000000_zinc_webhook_events_hardening.sql` (agregados `processing_status`, `processing_attempts`, `processing_error`, `last_processing_at`, y `processed_at` con default NULL).
- **Historial de Migraciones:** Local y Remote sincronizados en `20261230000000`.
- **Pruebas de Webhook en Vivo:**
  - `missing_signature`: HTTP 401 PASS
  - `invalid_signature`: HTTP 401 PASS
  - `valid_signature_placed`: HTTP 200 PASS
  - `deduplication`: HTTP 200 (`already_received: true`) PASS
  - `tracking_received`: HTTP 200 PASS
  - `delivered`: HTTP 200 PASS
  - `return_event_isolation`: HTTP 200 (`return_event: true`) PASS
  - `database_persistence`: Eventos persistidos con status, timestamp y sin errores PASS

---

## 4. Conclusión

La integración Zinc API V2 en Collectibles 2026 cuenta con certificación completa E2E en Sandbox, hardening de seguridad estricto, gestión de webhooks durable y atómica con HMAC-SHA256, y protección total del entorno de producción.

