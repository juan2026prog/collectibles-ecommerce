...
const AFTER_DIR = path.resolve(__dirname, 'phase2-after');
...
const BASE_URL = process.env.BASE_URL || 'http://localhost:4188';

async function captureScreenshots() {
  const browser = await chromium.launch({ headless: true });
  console.log(`Starting Phase 2 AFTER capture from ${BASE_URL}...`);
  ...
  for (const vp of VIEWPORTS) {
    ...
    try {
      ...
      await page.screenshot({ path: path.join(AFTER_DIR, `01_home_top_${vp.name}.png`) });
      console.log(`✓ 01_home_top_${vp.name}`);

      // 2. Home Hero / Catalog scroll
      await page.evaluate(() => window.scrollBy(0, 500));
      await page.waitForTimeout(1000);
      await page.screenshot({ path: path.join(AFTER_DIR, `02_home_catalog_${vp.name}.png`) });
      ...
      if (vp.isMobile) {
        ...
        if (await menuBtn.isVisible()) {
          ...
          await page.screenshot({ path: path.join(AFTER_DIR, `04_mobile_menu_${vp.name}.png`) });
      ...
      await page.waitForTimeout(1500);
      await page.screenshot({ path: path.join(AFTER_DIR, `06_shop_${vp.name}.png`) });
      ...
      if (vp.isMobile) {
        ...
        if (await filterBtn.isVisible()) {
          ...
          await page.waitForTimeout(800);
          await page.screenshot({ path: path.join(AFTER_DIR, `07_shop_filters_${vp.name}.png`) });
          console.log(`✓ 07_shop_filters_${vp.name}`);
        }
      }

      // 6. Licencias Index
      await page.goto(`${BASE_URL}/licencias`, { waitUntil: 'networkidle', timeout: 30000 });
      await page.waitForTimeout(1500);
      await page.screenshot({ path: path.join(AFTER_DIR, `08_licencias_${vp.name}.png`) });
      console.log(`✓ 08_licencias_${vp.name}`);

      // 7. Themes Index
      await page.goto(`${BASE_URL}/themes`, { waitUntil: 'networkidle', timeout: 30000 });
      await page.waitForTimeout(1500);
      await page.screenshot({ path: path.join(AFTER_DIR, `09_themes_${vp.name}.png`) });
      ...
      let pdpUrl = `${BASE_URL}/producto/neca-body-knocker-solar-power-marvel-dr-strange`;
      if (await firstProduct.isVisible()) {
        const href = await firstProduct.getAttribute('href');
        if (href) pdpUrl = `${BASE_URL}${href}`;
      }
      await page.goto(pdpUrl, { waitUntil: 'networkidle', timeout: 30000 });
      await page.waitForTimeout(1500);
      await page.screenshot({ path: path.join(AFTER_DIR, `10_pdp_top_${vp.name}.png`) });
      console.log(`✓ 10_pdp_top_${vp.name}`);

      // Scroll to CTA / Sticky Buy Bar
      await page.evaluate(() => window.scrollBy(0, 600));
      await page.waitForTimeout(1000);
      await page.screenshot({ path: path.join(AFTER_DIR, `11_pdp_cta_sticky_${vp.name}.png`) });
      ...
      if (await addToCartBtn.isVisible()) {
        ...
        await page.screenshot({ path: path.join(AFTER_DIR, `12_cart_drawer_${vp.name}.png`) });
      ...
      await page.waitForTimeout(1500);
      await page.screenshot({ path: path.join(AFTER_DIR, `13_cart_page_${vp.name}.png`) });
      console.log(`✓ 13_cart_page_${vp.name}`);

      // 10. Checkout Page
      await page.goto(`${BASE_URL}/checkout`, { waitUntil: 'networkidle', timeout: 30000 });
      await page.waitForTimeout(1500);
      await page.screenshot({ path: path.join(AFTER_DIR, `14_checkout_${vp.name}.png`) });
  ...
  await browser.close();
  console.log(`\nPhase 2 AFTER capture complete. Files saved to: ${AFTER_DIR}`);
}
