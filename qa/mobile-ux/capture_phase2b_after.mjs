...
const AFTER_DIR = path.resolve(__dirname, 'phase2b-after');
...
const BASE_URL = process.env.BASE_URL || 'http://localhost:4228';

async function captureScreenshots() {
  const browser = await chromium.launch({ headless: true });
  console.log('Starting Phase 2B AFTER capture from ' + BASE_URL + '...');
  ...
  await browser.close();
  console.log('\nPhase 2B AFTER capture complete. Files saved to: ' + AFTER_DIR);
}
