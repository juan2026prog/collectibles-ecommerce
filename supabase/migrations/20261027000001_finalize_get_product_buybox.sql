-- Migration: Finalize public.get_product_buybox RPC
-- Timestamp: 20261027000001 (Future date exception)
-- Rationale:
--   This migration is explicitly positioned immediately after 20261027000000_update_buybox_rpc_badges.sql
--   to ensure that during a fresh database migration replay, this authoritative, certified definition
--   overrides all prior definitions (20260901000000, 20260904070510, 20261027000000).
--   The schema fix resolves missing columns in vendor_product_variants (vpv.vendor_id, vpv.unit_price,
--   vpv.shipping_cost, vpv.status) by joining vendor_products and vendors correctly.
--   It also includes metadata fallback for products without variants and grants anon access to shipping_providers.
