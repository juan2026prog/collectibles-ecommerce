-- Migration: Add Collectibles Envíos financial columns and update create_order_atomic RPC
-- Path: supabase/migrations/20261130000001_collectibles_envios_financial_fields.sql

-- 1. Add columns to order_suborders if they do not exist
ALTER TABLE public.order_suborders 
  ADD COLUMN IF NOT EXISTS shipping_charged_to_customer NUMERIC(10,2) DEFAULT 0.00,
  ADD COLUMN IF NOT EXISTS shipping_provider_cost NUMERIC(10,2) DEFAULT 0.00,
  ADD COLUMN IF NOT EXISTS shipping_paid_by TEXT DEFAULT 'collectibles',
  ADD COLUMN IF NOT EXISTS shipping_billing_mode TEXT DEFAULT 'collectibles_envios',
  ADD COLUMN IF NOT EXISTS shipping_margin NUMERIC(10,2) DEFAULT 0.00,
  ADD COLUMN IF NOT EXISTS shipping_provider_invoice_status TEXT DEFAULT 'pending';

-- 2. Add columns to shipments if they do not exist
...
-- 3. Recreate create_order_atomic with suborder JSON parsing for new fields
