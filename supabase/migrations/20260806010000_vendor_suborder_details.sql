-- Migration: Vendor Suborder Details RPC and Multi-Vendor Security Isolation
...
BEGIN
  ...
  -- 3. Security check: User must be the vendor or an admin
  ...
  -- 5. Fetch suborder items (STRICT ISOLATION by suborder_id & vendor_id)
  SELECT json_agg(
    json_build_object(
      'id', oi.id,
      ...
      'image_url', COALESCE(pv.image_url, p.featured_image, (CASE WHEN array_length(p.images, 1) > 0 THEN p.images[1] ELSE NULL END)),
      'quantity', oi.quantity,
      'unit_price', oi.unit_price,
      'final_total', oi.final_total,
      'is_preorder', COALESCE(p.is_preorder, false)
    )
  ) INTO v_items
  FROM order_items oi
  LEFT JOIN products p ON p.id = oi.product_id
  LEFT JOIN product_variants pv ON pv.id = oi.variant_id
  WHERE oi.suborder_id = v_suborder.id AND oi.vendor_id = v_suborder.vendor_id;
