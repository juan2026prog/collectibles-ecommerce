-- Migration: Figuras de Acción Ultimate Guardrails with Action Figure Brands, Universes, and Exclusions
-- Date: 2026-07-15
  ...
  LOOP
    ...
    IF v_matched THEN
      ...
      IF v_rule.action_type = 'set_category' THEN
        ...
        -- Guardrail B: Action Figures (Figuras de Acción) cleanup - Ultimate positive signals & exclusions
        IF v_rule.action_target_id = 'ddd41421-fb1c-423f-a282-131aba8c4373' THEN
          ...
          v_has_positive_signal := (
            -- Signal A: Strong positive Action Figure keywords or lines
            p_title ~* '\y(action figures?|figuras? de acci[óo]n|articulated figures?|figuras? articuladas?)\y'
            OR p_title ~* '\y(marvel legends|black series|s\.?h\.? ?figuarts|myth cloth|mafex|figma|revoltech|neca|mcfarlane|super7|play arts|mezco|g\.?i\.? ?joe classified|motu origins|masters of the universe|motu|figuarts|nendoroid|retro collection|vintage collection|multiverse|ultimate edition|ultimates)\y'
            OR (p_title ~* '\y(transformers)\y' AND p_title ~* '\y(figuras?|figures?|action|articulada?s?)\y')
            OR
            -- Signal B: Recognized Action Figure Brand AND (Category is Action Figures OR has figure keywords)
            (
              (
                lower(COALESCE(p_brand_name, '')) IN ('neca', 'mcfarlane', 'figma', 'mafex', 'super7', 'mezco', 'bandai', 'hasbro', 'jakks pacific', 'dc collectibles', 'dc direct', 'playmates', 'playmates toys', 'medicom', 'medicom toy', 'medicos', 'medicos entertainment', 'jada', 'jada toys', 'storm collectibles', 'toy biz', 'diamond select', 'hot toys', 'boss fight studio', 'boss fight', 'kenner', 'mattel')
                OR p_title ~* '\y(hasbro|kenner|mattel|bandai|neca|mcfarlane|figma|mafex|super7|playmates|mezco|hot toys|revoltech|kotobukiya|kaiyodo|storm collectibles|diamond select|toy biz|jada toys|jakks pacific|boss fight)\y'
              )
              AND (p_title ~* '\y(figuras?|figures?|mu[ñn]ecos?|toys?)\y' OR p_ml_category_id = 'MLU176854')
            )
            OR
            -- Signal C: Action Universe keyword AND (Category is Action Figures OR has figure keywords)
            (
              p_title ~* '\y(marvel|x-?men|spiders?-?man|batman|superman|wonder woman|joker|avengers|star wars|transformers|g\.?i\.? ?joe|power rangers|he-man|motu|saint seiya|dragon ball|dbz|naruto|one piece|demon slayer|my hero academia|evangelion|gundam|macross|robotech|alien|predator|terminator|robocop|spawn|tmnt|tortugas ninja)\y'
              AND (p_title ~* '\y(figuras?|figures?|mu[ñn]ecos?)\y' OR p_ml_category_id = 'MLU176854')
            )
            OR
            -- Signal D: Action Universe keyword + Vintage/Retro/TAS/Classics lines
            (
              p_title ~* '\y(marvel|x-?men|xmen|star wars|transformers|he-man|motu|g\.?i\.? ?joe|saint seiya)\y'
              AND p_title ~* '\y(tas\d*|retro|vintage|classics?)\y'
            )
          );
          ...
          IF v_is_excluded OR NOT v_has_positive_signal THEN
            ...
            ELSE
              v_conflict_details := 'Producto no contiene señal positiva fuerte para Figuras de Acción (Marvel Legends, Black Series, SH Figuarts, Myth Cloth, Mafex, Figma, NECA, McFarlane, etc. o marcas/universos asociados).';
            END IF;
            ...
            action_type := v_rule.action_type;
            confidence := 30; -- Low confidence
            rule_description := concat(upper(v_rule.rule_type), ' Rule (Conflict - Figuras de Acción refined guardrail v2)');
            has_conflict := true;
            conflict_details := v_conflict_details;
            RETURN NEXT;
            RETURN;
