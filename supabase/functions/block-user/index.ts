...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { error: banError } = await supabaseAdmin.auth.admin.updateUserById(userId, {
        ban_duration: isBlock ? '8760h' : 'none'
    });

    if (banError) throw banError;

    // Additionally mark them as blocked in the DB
    await supabaseAdmin.from('profiles').update({ is_blocked: isBlock }).eq('id', userId).catch(() => {});
